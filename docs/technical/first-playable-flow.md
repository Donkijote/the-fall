...
The same panel exposes every current presentation preference: master audio, effects, music, fast
forward, and reduced motion. Settings is their only persistent control surface; the compact match HUD
applies the selected values and exposes only contextual Skip animation. The preferences remain active
for the current application session, affect presentation only, and do not enter deterministic match
state. Music remains intentionally source-free for the milestone.
...
Gateway, hub, Settings, match-action, and result symbols use the curated Game-icons.net PNG set
recorded in [UI icon library](../assets/ui-icon-library.md). UI Toolkit tints the shared
